...
const animatedValueOptions = computed<SlotTextOptions | null>(() => {
  if (!displayValue.value || animatedValue === false || slots.default) return null

  const compactMetricValue = /^[+\-$€£¥]?\s*[\d,.]+(?:\s?[kmbt])?(?:\.\d+)?%?$/i.test(displayValue.value)
  if (typeof animatedValue === 'object') return animatedValue

  return compactMetricValue && displayValue.value.length <= 14 ? { direction: 'up', duration: 320, stagger: 24 } : null
})
...
const isTrendPositive = computed(() => {
  if (trendDirection.value === 'neutral') return null
  return invertTrend ? trendDirection.value === 'down' : trendDirection.value === 'up'
})

const trendColorClass = computed(() => {
  if (!trendColored || isTrendPositive.value === null) return 'text-muted'
...
<template>
  <UiTooltip :text="tooltipDescription || tooltip" :disabled="!to || !tooltip" trigger-as="child">
    <component
      :is="rootTag"
      :to="to || undefined"
      data-ui="UiStat"
      class="relative flex flex-col"
      :class="[card ? 'gap-2' : 'gap-1', to ? 'cursor-pointer' : '']"
    >
      ...
      >
        ...
        <template v-else>
          ...
          <div
            class="relative z-10 flex items-end justify-between gap-3"
            :class="card && hasChart ? 'ui-stat-legible' : ''"
          >
            <slot>
              <div v-if="value !== undefined && value !== null" class="flex min-w-0 items-baseline gap-2">
              ...
              <span v-else class="font-semibold text-muted" :class="sizeConfig.value" role="img" aria-label="No data"
                >&mdash;</span
              >
            </slot>
            ...
            >
              <ClientOnly>
                <div v-if="$slots.chart" class="w-full" :style="{ height: `${sizeConfig.sparkH}px` }">
                  <slot name="chart" :hovered="cardHovered" />
                </div>
                <UiSparkline
                  v-else
