...
export function useChartTickPlan(opts: UseChartTickPlanOptions) {
  ...
  const tickPlan = computed<ChartTickPlan>(() => {
    ...
    if (len <= 1) return { indices: [0], format: (d) => monthDayFmt.format(d) }
    if (len <= 10) return { indices: dates.map((_, i) => i), format: (d) => weekdayDayFmt.format(d) }
    if (len <= 35) {
      const step = Math.max(1, Math.ceil(len / 5))
      const indices: number[] = []
      for (let i = 0; i < len; i += step) indices.push(i)
      if (indices.at(-1) !== len - 1) indices.push(len - 1)
    ...
    if (indices.length < 2) return { indices: [0, Math.floor(len / 2), len - 1], format: (d) => monthDayFmt.format(d) }
    ...
    if (indices.length > 2 && indices[1]! - indices[0]! < 5) indices.shift()
    ...
    if (indices.length > maxTicks) {
      ...
      return {
        ...
        format: (d, i, firstYear) =>
          (d.getUTCMonth() === 0 || i === 0) && d.getUTCFullYear() !== firstYear
    ...
    return {
      indices,
      format: (d, i, _firstYear) =>
        (d.getUTCMonth() === 0 && i > 0) || (i === 0 && d.getUTCMonth() !== 0 && indices.length > 6)
          ? monthYearFmt.format(d)
          : monthFmt.format(d),
    }
  })
  ...
  const firstTickYear = computed(() => {
    ...
    if (first === undefined || first === null || !dates[first]) return 0
